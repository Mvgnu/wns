 ...
 */
async function calculateGroupScore(group, user) {
    const score = {
        group,
        score: 0,
        reasons: [],
    ...
    if (user.latitude && user.longitude && group.latitude && group.longitude) {
        const distance = geolib_1.default.getDistance({ latitude: user.latitude, longitude: user.longitude }, { latitude: group.latitude, longitude: group.longitude });
    ...
    if (user.interestTags && group.groupTags) {
        const matchingTags = user.interestTags.filter(tag => group.groupTags.includes(tag));
