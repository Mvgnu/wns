...
async function sendEventReminderEmail(user, event) {
    try {
        ...
        const eventDate = (0, date_fns_1.format)(event.startTime, 'EEEE, MMMM do, yyyy');
        const eventTime = (0, date_fns_1.format)(event.startTime, 'h:mm a');
        const mailOptions = {
        ...
        <div>
          ${upcomingEvents.map(event => `
            <div style="padding: 10px; background-color: #f5f5f5; border-radius: 8px; margin-bottom: 10px;">
              ...
              <p><strong>Date:</strong> ${(0, date_fns_1.format)(event.startTime, 'EEEE, MMMM do, yyyy')}</p>
              <p><strong>Time:</strong> ${(0, date_fns_1.format)(event.startTime, 'h:mm a')}</p>
