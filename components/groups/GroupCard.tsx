...
export default function GroupCard({
	...
	const tags = Array.isArray(groupTags) ? groupTags.slice(0, 3) : [];
	const extraTags = Array.isArray(groupTags) && groupTags.length > 3 ? groupTags.length - 3 : 0;
			...
			<CardHeader className="pb-2">
				...
				{tags.length > 0 && (
					<div className="mt-1 flex flex-wrap gap-1">
						{tags.map((t) => (
							<span key={t} className="text-[10px] px-2 py-0.5 rounded bg-gray-50 border text-gray-700">{t}</span>
						))}
						{extraTags > 0 && (
							<span className="text-[10px] px-2 py-0.5 rounded bg-gray-50 border text-gray-700">+{extraTags}</span>
						)}
					</div>
				)}
			</CardHeader>
