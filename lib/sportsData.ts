// Sports data with hierarchical structure
...
export function getSportsByCategory() {
  const sportsByCategory: Record<string, Sport[]> = {};
  
  sportsCategories.forEach(category => {
    sportsByCategory[category] = allSports.filter(sport => sport.category === category);
  });
...
export function getSportByValue(value: string): Sport | undefined {
  return allSports.find(sport => sport.value === value);
}

// Function to get sports as options for select components
export function getSportsAsSelectOptions() {
