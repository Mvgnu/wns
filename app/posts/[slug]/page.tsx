...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // If found by ID but has a slug, redirect to slug URL
  if (post && post.slug && post.slug !== postParam) {
  ...
  return {
    ...
    alternates: {
      canonical: `/posts/${post.slug || post.id}`,
    },
  };
}
          ...
          <div className="flex items-center mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                ...
                {post.group && (
                  <> · Gepostet in <Link href={`/groups/${post.group.slug || post.group.id}`} className="text-blue-600 dark:text-blue-400 hover:underline">{post.group.name}</Link></>
                )}
              </p>
            </div>
          </div>
