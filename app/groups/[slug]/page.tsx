...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (!group) {
      ...
      const groupById = await prisma.group.findUnique({
        where: { id: groupSlug },
        select: {
          id: true,
          name: true,
      ...
      if (groupById && groupById.slug) {
        // Redirect to the proper slug URL
        redirect(`/groups/${groupById.slug}`);
      }
    ...
    const location = [group.city, group.state, group.country].filter(Boolean).join(', ');
    const tags = group.groupTags?.slice(0, 3).join(', ') || '';
    const description = group.description ||
      `${group.name} ist eine ${group.sport} Gruppe${location ? ` in ${location}` : ''}${tags ? `. ${tags}` : ''}. ${group.memberCount || 0} Mitglieder.`;

    return {
      title: `${group.name} - ${group.sport} Gruppe | WNS Community`,
      description: description,
      openGraph: {
        title: `${group.name} - ${group.sport} Gruppe`,
        description: description,
        type: 'website',
      },
      twitter: {
...
export default async function GroupPage({ params }: { params: { slug: string } }) {
    ...
    select: {
      ...
      members: {
        select: {
          ...
          image: true,
          groupMemberStatuses: {
            where: { groupId: { equals: '' } }, // We'll set this after we get the group
            select: {
              isAnonymous: true,
              status: true
            }
  ...
  if (!group) {
    ...
    // If found by ID but has a slug, redirect to the slug URL
    if (group && group.slug) {
  ...
  const requireApproval = entryRules.requireApproval || false;
  const contentModeration = settings.contentModeration || 'low';
  const allowMemberPosts = settings.allowMemberPosts !== false; // Default to true if not specified
  const allowEventCreation = settings.allowMemberEvents !== false; // Use allowMemberEvents from schema
              ...
              <div className="border-t border-gray-100 p-5">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center justify-between text-gray-600">
                    <span>Beitritt:</span>
                    <Badge variant="outline" className="bg-gray-50">
                      {requireApproval ? 'Mit Genehmigung' : 'Offen'}
                    </Badge>
                  </div>
                ...
                <div className="border-t border-gray-100 p-5">
                  ...
                  <div className="space-y-3">
                    <Button variant="outline" size="sm" className="w-full justify-start" asChild>
                      <Link href={`/groups/${group.slug || group.id}/edit`}>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
              <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                <h2 className="font-semibold text-gray-900">Gruppeninhalte</h2>
                  ...
                  {isMember && allowMemberPosts && (
                    <Button size="sm" asChild>
                      <Link href={`/groups/${group.slug || group.id}/posts/create`}>
                  ...
                  {(isMember && allowEventCreation) || isOwner || isAdmin ? (
                  ...
                  {group.posts.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500 max-w-md mx-auto mb-6">
                        Diese Gruppe hat noch keine Posts. {isMember && allowMemberPosts ? "Sei der Erste, der etwas teilt!" : ""}
                      </p>
                ...
                <TabsContent value="events" className="p-5">
                  {group.events.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      {((isMember && allowEventCreation) || isOwner || isAdmin) && (
                        <Button asChild>
                          <Link href={`/groups/${group.slug || group.id}/events/create`}>
                            Event planen
                          </Link>
                        </Button>
                      )}
                        ...
                        {group.events.length > 0 && (
                          <div className="text-center pt-4">
                            <Button variant="outline" asChild>
                              <Link href={`/events?groupId=${group.id}`}>
