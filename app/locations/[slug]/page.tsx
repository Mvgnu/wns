...
export async function generateMetadata({ params }: LocationPageProps): Promise<Metadata> {
  const locationParam = params.slug;

  // First try to find by slug, then by ID for backward compatibility
  let location = await prisma.location.findUnique({
    where: { slug: locationParam },
    select: { name: true, sport: true, type: true, slug: true }
  });

  // If not found by slug, try by ID
  if (!location) {
    location = await prisma.location.findUnique({
      where: { id: locationParam },
      select: { name: true, sport: true, type: true, slug: true }
    });
  }
  ...
  return {
    ...
    openGraph: {
      title: `${location.name} - Sport Location | WNS Community`,
      description: `Entdecke ${location.name}, eine ${location.type} für ${sportLabel}. Bewertungen, Details und Events an dieser Location.`,
      url: `/locations/${location.slug || locationParam}`,
    },
    twitter: {
    ...
    alternates: {
      canonical: `/locations/${location.slug || locationParam}`,
    },
  };
}
  ...
  try {
    ...
    if (userId) {
      ...
      const staffRecords = await prisma.$queryRaw`
        SELECT id FROM "PlaceStaff" WHERE "locationId" = ${location.id} AND "userId" = ${userId} LIMIT 1
      `;
      ...
      if (!userIsStaff) {
        const claims = await prisma.$queryRaw`
          ...
          WHERE "locationId" = ${location.id} AND "userId" = ${userId}
